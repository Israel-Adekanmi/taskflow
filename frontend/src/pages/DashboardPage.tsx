...
export default function DashboardPage({ auth, onLogout, onNavigate }: Props) {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await tasksApi.list(auth.token, filter !== 'all' ? filter : undefined);
      setTasks(data);
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-[#f0f2f7]">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-30">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-4">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-bold text-slate-900 text-lg tracking-tight" style={{ fontFamily: "'Instrument Serif', serif" }}>
              TaskFlow
            </span>
          </div>
          ...
          <div className="relative">
            ...
            >
              ...
              <span className="hidden sm:inline">{auth.user.firstName} {auth.user.lastName}</span>
              ...
              <>
                ...
                <div className="absolute right-0 mt-1 w-48 bg-white rounded-xl border border-slate-200 shadow-lg z-20 py-1 overflow-hidden">
                  <div className="px-3 py-2 border-b border-slate-100">
                    <p className="text-xs font-medium text-slate-900">{auth.user.firstName} {auth.user.lastName}</p>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        {/* Filters */}
        <div className="mb-5">
          <TaskFilters active={filter} onChange={(f) => { setFilter(f); }} />
        </div>
        ...
        {loading ? (
          ...
          <EmptyState
            title={filter === 'all' ? 'No tasks yet' : filterEmptyMessages[filter]}
            description={
              filter === 'all'
            ...
            action={filter === 'all' ? { label: 'Create Task', onClick: () => onNavigate({ name: 'create-task' }) } : undefined}
          />
        ) : (
          <div className="space-y-3">
            {tasks.map(task => (
              <TaskCard
                key={task.id}
                task={task}
                onView={() => onNavigate({ name: 'task-detail', taskId: task.id })}
                onEdit={() => onNavigate({ name: 'edit-task', taskId: task.id })}
      ...
      {deleteTarget && (
        <DeleteTaskDialog
          task={deleteTarget}
          token={auth.token}
          onDeleted={() => { setDeleteTarget(null); fetchTasks(); }}
